...
function removeFirstOccurrences(str, substr) {
  ...
  if (index < 0 && !str.length) {
    // Yes, this is useless peace of code, because for this task exist specified method with only purpose to remove first occurence of substring!
    // It's like "Oooh, I want you to make me a set without using a new Set()". Why?.."
    return '';
  }
...
function removeLastOccurrences(str, substr) {
  ...
  return lastIndex < 0
    ? str
    : str.slice(0, lastIndex) + str.slice(lastIndex + substr.length);
}
...
function isPalindrome(str) {
  ...
  const filterStr = str
    ...
    .join('');
  const revStr = filterStr.split('').reverse().join('');
...
function encodeToRot13(str) {
  ...
  return str
    .split('')
    .map((el) =>
      alphabet.includes(el) ? rotAlphabet[alphabet.indexOf(el)] : el
    )
    .join('');
}
...
function getCardId(card) {
  ...
  const suitIndex = suits.indexOf(card.slice(-1));
